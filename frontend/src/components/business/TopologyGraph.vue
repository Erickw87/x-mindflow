...
function renderGraph() {
  ...
  const g6Nodes = nodes.map((node, index) => {
    ...
    if (isService) {
      ...
      if (branchMatch) {
        serviceName = branchMatch[1]; // 提取服务名，如 "service-a"
        versionLabel = `branch${branchMatch[2]}`; // 提取版本名，如 "branch1"
      }
  ...
  const g6Edges = edges.map((edge) => {
    ...
    // 检查是否是隐藏边
    const isHidden = (edge as any).hidden === true;
  ...
  const animate = () => {
    ...
    edges.forEach((edge) => {
      graph.updateEdgeData([{
        id: edge.id,
        style: {
          lineDashOffset: -offset, // 负值使虚线向前移动
        },
      }]);
    });
